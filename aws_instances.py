...
if(args.verbose):
	myData = [['public_ip','private_ip','dns','is_active','name','created_at','instance_id','groups']]
else:
	myData = [['public_ip']]
...
try:
	...
	for data in json_data:
		if(str(data['ip']) != 'None' and str(data['is_active']) == 'True'):
			if(args.verbose):
				myData = [[str(data['ip']),str(data['private_ip']),str(data['dns']),str(data['is_active']),str(data['name']),str(data['created_at']),str(data['instance_id']),str(data['groups'])]]
				myFile = open('aws_instances_' + timestamp + '.csv', 'a')
				with myFile:
					writer = csv.writer(myFile)
					writer.writerows(myData)
				print(str(data['ip']) + ',' + str(data['private_ip']) + ',' + str(data['dns']) + ',' + str(data['is_active']) + ',' + str(data['name']) + ',' + str(data['created_at']) + ',' + str(data['instance_id']) + ',' + str(data['groups']))
			else:
				myData = [[str(data['ip'])]]
				myFile = open('aws_instances_' + timestamp + '.csv', 'a')
				with myFile:
